...
- **An action that is not wired up renders disabled, never as a link to nowhere** [PRINCIPLE]. While a checkout URL, form endpoint, or route is still a placeholder, the control must be `disabled` with a title saying what is missing — a button that looks live and does nothing costs more than a visibly unfinished one, because it fails silently and only in front of a real user.
...
- **Which image goes in which slot is approved, not assumed** [PRINCIPLE]. Run `asset-inventory.mjs` over the project's images, open each one and say what it shows, and print the assigned filename in its slot on the mockup. The mechanical half — ratio against the slot, resolution against the rendered size, orphan files, empty slots — is measured; the half that needs eyes is yours; the result is confirmed by the person who owns the pictures.
- **Where images come from is a design decision** [PRINCIPLE]. Three sources, in order of preference: the real thing (photograph the product, screenshot the app, use the actual cover), commissioned or licensed work, and generated imagery. Generated images are legitimate for atmosphere — texture, an abstract backdrop, a mood shot with no claim to be a record of anything. They are **not** legitimate wherever the image asserts a fact: a real person's portrait, a real product, a real place, a book's actual cover, a screenshot of a UI that exists, a photograph offered as evidence. The test is whether a viewer would feel deceived on learning how the image was made. When a factual image is missing, ship the placeholder and name it in the shopping list — never fill the hole with a plausible fake.
- **Crop deliberately, and keep the set consistent.** Decide the focal point and hold it across breakpoints (`object-position`, or separate art-directed sources via `<picture>`) — a hero cropped by chance decapitates someone on mobile. Across a grid, one aspect ratio, one treatment, one light: mixed crops and mixed color grading read as broken layout even when every individual image is good.
- **Dark mode is not a filter over the images.** Logos need a light-background variant or a transparent version with adequate padding; photographs with white studio backgrounds glare against a dark canvas and need either a different asset or a contained frame; screenshots of a light UI must be framed, never inverted. Decide per asset, and record the decision.
