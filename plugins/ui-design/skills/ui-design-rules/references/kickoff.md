...
1. Compose 3 palette candidates and 3–4 type pairings from the intent (visual-directions.md sets the character, color.md the construction). Make them differ in *character*, not in decimals, and spread them on **two axes, not one**:
   - **Energy** — one quieter than the brief, one at it, one a notch louder. Three variants of safe is the classic failure: the user picks the middle of a distribution whose entire width is beige.
   - **Premise** — at least one candidate must challenge the leading interpretation of the brief, not just its volume. Three warm-paper palettes perfectly spread on energy are still one reading shown three times, and if that reading is wrong (it codes the wrong industry, the wrong price point), the whole round is lost on an axis nobody varied. A full round of renders is the most expensive way to discover a premise error.

   Every candidate carries a `reads` line — *what its register codes as on this market* ("boutique publisher", "IT integrator", "modern consultancy") — filled in the brief's language; the specimen renders it on the card. Category-coding is critiqued at the gate, before pixels are approved, not discovered after a rejection. If an asset exists, the hues measured in Stage 0 constrain every candidate (color.md, "Palette from an asset").
