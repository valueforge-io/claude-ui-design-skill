...
- **Unfilled copy must look unfilled in the running page** [PRINCIPLE]. Keep the bracket convention through to render time and give bracketed strings a visible marker — a dashed underline is enough (`isOwed(s) => s.startsWith("[")`). Placeholder text that renders like finished text becomes permanent, because nobody can see the difference on the page they are approving; text that visibly reads as debt gets paid.
- **Consent and privacy copy blocks publication, it is not ordinary debt** [STANDARD]. The moment a form collects personal data, the consent clause, the identity of the data controller, and a real privacy policy stop being placeholders you can ship around — the page is not publishable without them, and a checkbox alone does not constitute consent. Say so explicitly when handing over, separately from the content shopping list, because the person receiving it may not know it is a legal blocker rather than a nice-to-have.
