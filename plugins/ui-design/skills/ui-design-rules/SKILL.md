...
- **Contrast [STANDARD]:** body text ≥4.5:1, large text and UI elements ≥3:1 — `node scripts/contrast-check.mjs page.html`. Static text is composited against its DOM backdrop; text inside sticky/fixed bars with translucent backgrounds is measured **from rendered pixels at every scroll position it can occupy**, because the DOM cannot say what a pinned bar floats over. Thresholds use the effective rendered size (an SVG label scaled down by its viewBox is body text, whatever its declared px). Never ship a silent violation.
...
- **Fonts [STANDARD]:** `node scripts/font-check.mjs page.html` — every family used by visible text really loaded (width-probe; `document.fonts.check()` says yes to nonexistent families), the language's glyphs are actually in the loaded faces, and every weight/style in use has a real face behind it — no faux italic, no faux bold (typography.md, "Provisioning").
...
- **Negative-test every gate** [PRINCIPLE]: a probe that can only say yes must be shown able to say no before its yes means anything. Validate each check once against input known to be wrong — a fake font family, a deliberately failing palette, a page with a planted violation. Every verification script here self-tests or was calibrated against a page built to fail; extend the same courtesy to any check you write ad hoc.
